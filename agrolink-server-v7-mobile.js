...
<html lang="tr"><head><meta charset="UTF-8"><title>Kullanıcı Bulunamadı • AgroLink</title>
<meta name="viewport" content="width=device-width,initial-scale=1">
...
</head><body><div class="box"><div class="logo">🌾 AgroLink</div><h2>Kullanıcı bulunamadı</h2><p>Bu profil mevcut değil veya silinmiş olabilir.</p>
<a href="${DOMAIN}">Ana Sayfaya Dön</a></div></body></html>`);
        }
        ...
        const userTypeTr = user.userType === 'farmer' ? '👨‍🌾 Çiftçi' : user.userType === 'expert' ? '🔬 Uzman' : '🌱 Üye';

        // Son gönderiler
        const recentPosts = await dbAll(
            `SELECT id, content, "mediaUrls", "mediaType", "likeCount", "commentCount", "createdAt"
             FROM posts WHERE "userId"=$1 AND "isActive"=TRUE ORDER BY "createdAt" DESC LIMIT 6`,
            [user.id]
        ).catch(() => []);
        ...
        const postsHtml = (recentPosts || []).map(p => {
            ...
            if (mediaSrc && p.mediaType !== 'video') {
                mediaHtml = `<img src="${mediaSrc}" alt="gönderi" style="width:100%;height:160px;object-fit:cover;border-radius:12px 12px 0 0">`;
            } else if (mediaSrc && p.mediaType === 'video') {
                mediaHtml = `<div style="background:#0a1f10;height:160px;display:flex;align-items:center;justify-content:center;border-radius:12px 12px 0 0;font-size:40px">▶️</div>`;
            }
            const content = (p.content || '').substring(0, 80) + ((p.content || '').length > 80 ? '...' : '');
            return `<a href="${DOMAIN}/p/${p.id}" style="text-decoration:none">
  <div style="background:#0d1f14;border:1px solid rgba(0,230,118,0.1);border-radius:12px;overflow:hidden;transition:transform .2s">
    ${mediaHtml}
    <div style="padding:12px">
      ${content ? `<p style="color:#e8f5e9;font-size:13px;margin-bottom:8px">${content}</p>` : ''}
      <span style="color:#00e676;font-size:12px">❤️ ${p.likeCount||0} &nbsp; 💬 ${p.commentCount||0}</span>
    </div>
  </div></a>`;
        }).join('');
...
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Segoe UI',Arial,sans-serif;background:#060d0a;color:#e8f5e9;min-height:100vh}
.cover{width:100%;height:200px;object-fit:cover;background:linear-gradient(135deg,#0a1f10,#0d2b16)}
.container{max-width:680px;margin:0 auto;padding:0 16px 40px}
.profile-card{background:#0d1a12;border:1px solid rgba(0,230,118,0.12);border-radius:20px;padding:24px;margin-top:-40px;position:relative}
.avatar{width:90px;height:90px;border-radius:50%;border:3px solid #00e676;object-fit:cover;background:#0a1f10}
.name{font-size:22px;font-weight:800;margin-top:12px}
.username{color:#00e676;font-size:14px;margin:2px 0 8px}
.badge{display:inline-block;background:rgba(0,230,118,0.1);color:#00e676;border:1px solid rgba(0,230,118,0.3);border-radius:20px;padding:3px 10px;font-size:12px;margin-bottom:10px}
.bio{color:rgba(255,255,255,0.65);font-size:14px;line-height:1.6;margin-bottom:16px}
.stats{display:flex;gap:24px;margin-bottom:20px}
.stat{text-align:center}.stat-num{font-size:20px;font-weight:800;color:#00e676}.stat-label{font-size:12px;color:rgba(255,255,255,0.4)}
.cta{display:inline-block;padding:12px 28px;background:linear-gradient(135deg,#00e676,#1de9b6);color:#020810;font-weight:800;border-radius:50px;text-decoration:none;font-size:15px}
.posts-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:12px;margin-top:24px}
.section-title{font-size:16px;font-weight:700;color:#00e676;margin:28px 0 12px;border-bottom:1px solid rgba(0,230,118,0.15);padding-bottom:8px}
.brand{text-align:center;margin-top:32px;color:rgba(255,255,255,0.25);font-size:13px}
.brand span{color:#00e676;font-weight:700}
</style>
</head>
<body>
${user.coverPic ? `<img class="cover" src="${user.coverPic.startsWith('http') ? user.coverPic : DOMAIN + user.coverPic}" alt="kapak">` : '<div class="cover"></div>'}
<div class="container">
  <div class="profile-card">
    ...
    <span class="badge">${userTypeTr}</span>
    ${user.bio ? `<p class="bio">${user.bio}</p>` : ''}
    <div class="stats">
      <div class="stat"><div class="stat-num">${user.postCount || 0}</div><div class="stat-label">Gönderi</div></div>
      <div class="stat"><div class="stat-num">${user.followerCount || 0}</div><div class="stat-label">Takipçi</div></div>
    </div>
  ...
  <div class="brand">🌾 <span>AgroLink</span> — Tarım Topluluğu</div>
</div>
</body>
</html>`);
    } catch (e) {
        console.error('[PROFIL SAYFASI] Hata:', e.message);
        res.redirect('/');
    }
});
